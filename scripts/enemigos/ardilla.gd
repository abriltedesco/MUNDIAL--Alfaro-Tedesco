...
func _on_zona_dialogo_body_entered(body: Node2D) -> void:
	if body.name == "nani" and not hablando and not Progreso.minijuegoGanado(4):
		hablando = true
		body.movIntro = true
		body.play("default")
		...
		escenaGlobo.start(recurso, "charla_ardilla", [self])

func irAlMinijuego() -> void:
	Input.set_default_cursor_shape(Input.CURSOR_ARROW) 
	get_tree().change_scene_to_file("res://minijuegos/smashCastor/escena/juego.tscn")
