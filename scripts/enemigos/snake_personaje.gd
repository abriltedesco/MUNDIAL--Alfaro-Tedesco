...
func _on_zona_dialogo_body_entered(body: Node2D) -> void:
	if body.name == "kiki" and not hablando and not Progreso.minijuegoGanado(1):
		hablando = true
		body.movIntro = true
		body.play("default")
		...
		escenaGlobo.start(recurso, "charla_serpiente", [self])

func irAlMinijuego() -> void:
	Input.set_default_cursor_shape(Input.CURSOR_ARROW) 
	get_tree().change_scene_to_file("res://minijuegos/snake/snake.tscn")
